...
2020/09/12
	根据个人理解，输出交变电流不一定非得要正弦波，矩形波也行，后面添加一个备用版本产生频率占空比可调的PWM
	板载	OC1：	TIM6控制GPIO输出
				OC2:	TIM8互补输出
				OC3：	TIM3输出
	14:35发现：定时器映射管脚没有对应的组合，方案失败
*****************************************************************************************************/	
...
//	PWMOC1_Time6Init();
//	PWMOC2_Time8Init();//互补输出PWM定时器初始化																【测试通过，但发现互补PWM不能产生SPWM】
//	PWMOC3_Time3Init();
...
void TaskEncodeUpdate_Freq(u16 nowcount)
{
	...
	if(nowcount < LastcountFreq)//频率减小	
	{
		...
		if(NowFreqOut[NowSelect-1] <FreqMin)NowFreqOut[NowSelect-1]=FreqMin;//往下限幅
		else if(NowFreqOut[NowSelect-1] >FreqMax)NowFreqOut[NowSelect-1]=FreqMin;//数据往下溢出，所以仍然往下限幅
	}
	else//频率增大
	{
		...
		if(ValueChanged>EncodeMaxChange) return;//一定时间内改变量太大，判断为计数器溢出，忽略本次操作
		NowFreqOut[NowSelect-1] += ValueChanged*StepFreq;
		if(NowFreqOut[NowSelect-1] >FreqMax)NowFreqOut[NowSelect-1]=FreqMax;//往上限幅
	}
...
void TaskEncodeUpdate_Ampl(u16 nowcount)
{
	...
	if(nowcount < LastcountAmpl)//幅值减小
	{
		...
		if(NowAmplOut[NowSelect-1] <AmplMin)NowAmplOut[NowSelect-1]=AmplMin;//往下限幅
		else if(NowAmplOut[NowSelect-1] >AmplMax)NowAmplOut[NowSelect-1]=AmplMin;//数据往下溢出
	}
	else//幅值增大
	{
		...
		if(ValueChanged>EncodeMaxChange) return;//一定时间内改变量太大，判断为计数器溢出，忽略本次操作
		NowAmplOut[NowSelect-1] += ValueChanged*StepAmpl;	
		if(NowAmplOut[NowSelect-1] >AmplMax)NowAmplOut[NowSelect-1]=AmplMax;//往上限幅
	}
	Flag_TaskDisplayUpdate=1;//数据发生改变，允许更新显示
	Flag_TaskDataDtorage=1;//参数被修改，允许记录
	Flag_TaskOutputControl=1;
