...
uint8_t I2CTemp_Wait_Ack(void)
{
	uint8_t ucErrTime=0;
	TempSDA_IN();      //SDA设置为输入  
	I2CTemp_SDA = 1;delay_us(us1);	   
	I2CTemp_SCL = 1;delay_us(us1);	 
	while(ReadTemp_SDA)
	{
		ucErrTime++;
		if(ucErrTime>250)
...
{                        
    ...
    {              
        ...
        txd<<=1; 	  
		delay_us(us2);   //对TEA5767这三个延时都是必须的
		I2CTemp_SCL = 1;
		delay_us(us2); 
		I2CTemp_SCL = 0;	
		delay_us(us2);
...
float ReadTemperature(u8 deviceAddr)
{
	...
	I2CTemp_Start();//起始信号
	I2CTemp_Send_Byte(deviceAddr & 0xfe);//高7位位设备地址，bit0拉低为写操作
	I2CTemp_Wait_Ack();
	I2CTemp_Send_Byte(0x00);//发送寄存器地址
	I2CTemp_Wait_Ack();
	
	I2CTemp_Start();//起始信号
	I2CTemp_Send_Byte(deviceAddr | 0x01);//高7位位设备地址，bit0拉高为读操作
	I2CTemp_Wait_Ack();
	Data_MSB = I2CTemp_Read_Byte(1);//需要主机发送一个应答信号
	Data_LSB = I2CTemp_Read_Byte(0);
	I2CTemp_Stop();
	
	ReadData = ((u16)Data_MSB<<8) + Data_LSB;
	ReadData >>=5;//低5位是无效值
	
	if(Data_MSB&0x80)//不为0，表示温度为负
	{
		ReadData &= 0x0bff;//先忽略符号位
		ReadData = ~ReadData;//按位取反
		ReadData += 0x0001;//得到补码
		temp = ReadData*0.125;//计算温度的绝对值
		if(temp >40) return 0;//温度小于 -10℃无效
		return -temp;//返回负数
	}
	...
	{
		temp = ReadData*0.125;
		if(temp >120) return 0;//温度大于 120℃无效
//		temp -=30;//负数测试，正式代码需要注释【测试通过】
		return temp;
	}
}
