...
//	printf("Measure vlotage is: %f V\r\n",MeasureVoltage);//【测试用，记得注释掉】
	MeasureVoltage += 0.535;//矫正
//	printf("Count vlotage is: %f V\r\n",MeasureVoltage);//【测试用，记得注释掉】
	Current_A = MeasureVoltage*10 - 25;//根据ACS712的U-I曲线做数据转换【 U = 2.5 + I/10】即【I = 10*U - 25】
	return (u16) fabs(Current_A*1000);//返回电流值单位为mA,且只取模值
}
